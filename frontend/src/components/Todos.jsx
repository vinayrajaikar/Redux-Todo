...
const Todos = () => {
  // Adjust to access state.todo.todos
  const todos = useSelector((state) => state.todo.todos);
  const dispatch = useDispatch();

  // State to track the editable todo
  ...
  const handleEdit = (todo) => {
    setEditId(todo.id);
    setEditText(todo.text); // Set the current text in input
  };
  ...
  return (
    <>
      <div className="text-center font-bold text-2xl my-6">Todos</div>
      <ul className="list-none mx-20">
        {todos.map((todo) => (
          <li key={todo.id} className="mt-4 flex justify-between items-center bg-zinc-800 px-4 py-2 rounded shadow-lg">
            {/* If in edit mode, show input, else show text */}
            {editId === todo.id ? (
              <input
                value={editText}
